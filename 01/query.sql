...
delete from usuarios where (email, id) in (select email, max(id) from usuarios group by email having count(*) > 1);
...
--7
update usuarios set senha = 'k9P31H1O' where email = 'Duis@eget.ca';
